...
interface KubernetesClusterService {
  ...
  /**
   * Checks to see if a cluster with the provided name is active. This can be a very expensive query that
   * attempts to poke the API endpoint on a Kubernetes cluster to determine if the cluster is running. Implementations
   * may perform a reasonable amount of caching to improve performance.
   *
