...
class Kops(private val config: KopsConfig) {
    ...
    fun deleteCluster(delete: DeleteFabric) {
        val res = kops("delete", "cluster", delete.clusterName, "--yes", "--state=s3://${config.stateStore}")
        if (res.exitValue == 0) {
            logger.info("Kubernetes cluster deleted'{}'", delete.clusterName)
        } else {
            throw RuntimeException("Unable to delete cluster '${delete.clusterName}'")
        }
    }
