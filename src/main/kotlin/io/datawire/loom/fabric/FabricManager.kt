...
class FabricManager : BaseVerticle<LoomConfig>(LoomConfig::class) {
    ...
    override fun start() {
        vertx.eventBus().localConsumer<FabricModel>("fabric-model.create").handler(this::createFabricModel)
        vertx.eventBus().localConsumer<Fabric>("fabric.create").handler(this::createFabric)
    }

    fun createFabricModel(msg: Message<FabricModel>) {
        val model = msg.body()
        logger.info("Create fabric model ${model.name} ${model.version}")

        if (!workspace.directoryExists("fabric-models/")) {
            workspace.createDirectoryBlocking("fabric-models/")
        }

        val fsPath = "fabric-models/${model.name}-v${model.version}.json"
        workspace.writeJsonFileBlocking(fsPath, model)

        msg.reply(model.copy(creationTime = Instant.now()))
    }

    fun deleteFabric(msg: Message<String>) {
